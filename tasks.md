...
| 2026-07-23 | v2.7 | SearXNG 警報連環修復（Opus 4.8 獨立確認診斷）：`wall-e/hermes-healthcheck.sh` 首次納入版控（先前為 Wall.E 本機未追蹤獨立檔案，不在 hermes-mesh repo 掃描範圍內，故昨天 `.env` 修復未觸及）；修正兩層問題——① 寫死 dead port 8888 → 改 9119；② 改 9119 後仍失敗，因腳本用 Wall.E 自己的 Tailscale IP 打自己而 SearXNG 只綁 localhost，改為 `http://localhost:9119`。手動執行 healthcheck 腳本確認不再誤報。README 檔案結構新增此檔案項目。 | Ken + Claude（Sonnet 5，Opus 4.8 診斷確認） |
